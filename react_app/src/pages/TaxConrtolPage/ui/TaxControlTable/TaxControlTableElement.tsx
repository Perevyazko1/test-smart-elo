...
import {product} from "entities/OrderProduct";
...
export const TaxControlTableElement = memo((props: TaxControlTableElementProps) => {
    ...
    const [currentTax, setCurrentTax] = useState(tax_control_item.tax)
        ...
        >
            ...
            <td>
                <Input
                    type={'number'}
                    style={{width: "100px"}}
                    defaultValue={tax_control_item.tax}
                    onChange={(e) => {
                        setCurrentTax(Number(e.target.value))
                    }}
                />
            </td>
            <td>{tax_control_item.confirmation_date || "Тариф не утвержден"}</td>
            <td>
                {tax_control_item.approved_by
                    ?
                    <>
                        {tax_control_item.approved_by.first_name}
                        {tax_control_item.approved_by.last_name}
                    </>
                    :
                    "Тариф не утвержден"
                }
