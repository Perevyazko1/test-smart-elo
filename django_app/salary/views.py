...
from .utils import annotate_payroll_rows
...
def confirm_earnings(request):
    ...
    # Использование новой функции для аннотации
    queryset = PayrollRow.objects.filter(
        user=user_id,
        payroll__date_from=date_from,
        payroll__date_to=date_to,
    )

    annotated_queryset = annotate_payroll_rows(queryset)
    target_row = annotated_queryset.first()

    if not target_row:
        return Response(
            {'error': 'PayrollRow not found'},
            status=status.HTTP_404_NOT_FOUND
        )

    return Response(
        PayrollRowSerializer(target_row).data,
...
def close_payroll_row(request):
    ...
    return Response(
        status=status.HTTP_200_OK,
        data=PayrollRowSerializer(payroll_row).data
    )
...
class PayrollRowViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return annotate_payroll_rows(queryset)
