...
class GetWeekInfo:
    ...
    def _get_initial_values(self):
        ...
        if not self.week or not self.week.isdigit():
            self.week = str(iso_week)

        if not self.year or not self.year.isdigit():
            self.year = str(iso_year)
    ...
    def execute(self) -> WeekInfo:
        ...
        for day in range(7):
            ...
            str_dates.append(current_day.strftime('%d.%m'))
            dt_dates.append(current_day)

        range_first = dt_dates[0]
        range_last = dt_dates[-1]
        date_range = [range_first, range_last]

        prev_date = first_day - timedelta(days=7)
        ...
        previous_week_data = {"week": str(prev_week), "year": str(prev_year)}
        next_week_data = {"week": str(next_week), "year": str(next_year)}
