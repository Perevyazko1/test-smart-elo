...
def _build_chart_grid():
    ...
    for dept in topo_depts:
        ...
        if not is_dependent:
            ...
            #
            # Сортировка: по weight (одинаковый для всех цехов).
            # Weight уже учитывает слайдеры (сроки, прогресс, загрузка, обр.связь).
            # Все стартовые цеха работают в одном порядке по весу.
            #
            # bottleneck_score НЕ используется для стартовых цехов, потому что
            # он ставит маленькие заказы (подушки) первыми во ВСЕХ цехах,
            # но подушки проходят только через Крой→Пошив. В итоге Крой занят
            # подушками, а Пила делает другие заказы → пути расходятся → дырки.
            #
            # Дырки в зависимых цехах закрывает день-по-день раскладка:
            # Обивка каждый день берёт ВСЁ что готово от предшественников.
            # ═══════════════════════════════════════════════════════════════
            dept_orders = [o for o in order_dept_data if dept in o['depts']]

            # Сортировка по весу — единый порядок для всех стартовых цехов
            dept_orders.sort(key=lambda o: o['weight'], reverse=True)
            ...
            #
            # Сортировка: weight + readiness бонус (до +30% от weight).
            # weight — единый приоритет (сроки, прогресс, обр.связь).
            # readiness бонус — если от предшественников пришло много деталей,
            # даём небольшой бонус чтобы цех не простаивал.
            # load_factor масштабирует бонус: при высокой Загрузке бонус больше.
            # ═══════════════════════════════════════════════════════════════
            ...
            while day < MAX_DAYS:
                ...
                # Сортировка зависимых цехов: weight + readiness бонус
                # Основной приоритет — weight (одинаковый для всех цехов).
                # readiness бонус (0..weight*0.3) — если от предшественников пришло
                # много деталей, даём небольшой бонус чтобы не простаивал цех.
                # bottleneck_score УБРАН — он давал маленьким заказам (подушки)
                # высокий приоритет во всех цехах, что десинхронизировало потоки.
                max_readiness = max(r for _, _, r in scorable) or 1
                scorable.sort(
                    key=lambda x: (
                        x[0]['weight'] +
                        x[2] / max_readiness * x[0]['weight'] * 0.3 * load_factor
                    ),
                    reverse=True,
                )
